 ...
 * direkt aus dem aktuellen Helden (XML-API), ohne externes Python-Skript.
 */
public class ModernBogenPlugin implements HeldenXMLDatenPlugin3 {
    ...
    public ModernBogenPlugin() {
        super();
    }

    @Override
    public String getMenuName() {
        return "Helden-Overhaul";
    }

    @Override
    public String getToolTipText() {
        return "Erzeugt einen modernen HTML-Charakterbogen (Fantasy-Layout, Würfel, Dark Mode)";
    }
    ...
    private File exportModernBogen() {
        ...
        if (heldDoc == null) {
            JOptionPane.showMessageDialog(frame,
                    "Kein Held geladen oder XML konnte nicht gelesen werden.",
                    "Fehler", JOptionPane.ERROR_MESSAGE);
            return null;
        }

        String heldName = HtmlGenerator.extractHeldName(heldDoc);
        String defaultName = sanitizeFilename(heldName) + "_modern.html";
        ...
        if (!htmlFile.getName().toLowerCase().endsWith(".html")
                && !htmlFile.getName().toLowerCase().endsWith(".htm")) {
    ...
    private Document getCurrentHeldenXml() {
        ...
        // Offizielle API: action=held, id=selected, format=xml, version=2 → <daten>…<zauberliste>
        ...
        for (int i = 0; i < variants.length; i++) {
            try {
                ...
                if (variants[i][3].length() > 0) {
                    action.setAttribute("version", variants[i][3]);
                }
                ...
                int score = 0;
                score += doc.getElementsByTagName("zauber").getLength() * 10;
                score += doc.getElementsByTagName("zauberliste").getLength() * 5;
                score += doc.getElementsByTagName("talent").getLength();
                score += doc.getElementsByTagName("talentliste").getLength();
                if (doc.getDocumentElement() != null
                        && "daten".equalsIgnoreCase(doc.getDocumentElement().getTagName())) {
                    score += 2;
                }
                if (score > bestScore) {
                    bestScore = score;
    ...
    private static void copyResource(String resourceName, File target) throws Exception {
        ...
        if (in == null) {
            in = ModernBogenPlugin.class.getResourceAsStream("/resources/" + resourceName);
        }
        if (in == null) {
            // Fallback: CSS aus dem gleichen Classpath-Root
            in = ModernBogenPlugin.class.getClassLoader().getResourceAsStream(resourceName);
        }
        if (in == null) {
            throw new IllegalStateException("Ressource nicht gefunden: " + resourceName);
        }
        try {
