...
/** Fernkampf-Parität: XML für ausgerüstete FK-Waffen/Talent, native HTML für Waffenwerte. */
public final class FernkampfParityEnhancer {
    ...
    public static String enhance(String html, Document doc, String nativeHtml) {
        ...
        Map<String, NativeRow> rows = nativeHtml == null ? new HashMap<String, NativeRow>() : parseNativeRows(nativeHtml);
        mergeNative(weapons, rows);
        if (weapons.isEmpty()) return html;

        String section = render(weapons);
        String lower = html.toLowerCase();
        int existing = lower.indexOf("section-fernkampfwaffen-10");
        if (existing >= 0) {
            int start = lower.lastIndexOf("<table", existing);
            if (start >= 0) {
                int end = findTableEnd(html, start);
                if (end > start) return html.substring(0, start) + section + html.substring(end);
            }
        }
    ...
    private static int findTableEnd(String html, int start) {
        Matcher m = Pattern.compile("(?is)</table>").matcher(html);
    ...
    private static List<RangedWeapon> read(Document doc) {
        ...
        for (int i = 0; i < equipped.getLength(); i++) {
            ...
            boolean explicitFk = id.toLowerCase().startsWith("fkwaffe");
            if (!explicitFk && weaponName.isEmpty()) continue;
            if (talent.isEmpty()) talent = findWeaponTalent(doc, weaponName);
            if (!explicitFk && talent.isEmpty()) continue;
    ...
    private static RangedWeapon create(Document doc, Element equipment, String weaponName, String talent) {
        ...
        if (w.minbf.isEmpty()) w.minbf = attr(equipment, "bfmin", "minbf");
        if (w.aktbf.isEmpty()) w.aktbf = attr(equipment, "bfakt", "aktbf", "bf");
        return w;
    }
    ...
    private static String findWeaponTalent(Document doc, String weaponName) {
        Element e = findWeaponData(doc, weaponName);
    ...
    private static void mergeNative(List<RangedWeapon> weapons, Map<String, NativeRow> rows) {
        for (RangedWeapon w : weapons) {
            ...
            if (r == null) continue;
            if (!r.tp.isEmpty()) w.tp=r.tp; if (!r.tpkk.isEmpty()) w.tpkk=r.tpkk; if (!r.tpEntfernung.isEmpty()) w.tpEntfernung=r.tpEntfernung;
            if (!r.reichweite.isEmpty()) w.reichweite=r.reichweite; if (!r.lz.isEmpty()) w.lz=r.lz; if (!r.ini.isEmpty()) w.ini=r.ini;
            if (!r.mod.isEmpty()) w.mod=r.mod; if (!r.munition.isEmpty()) w.munition=r.munition; if (!r.minbf.isEmpty()) w.minbf=r.minbf; if (!r.aktbf.isEmpty()) w.aktbf=r.aktbf;
        }
    }
    ...
    private static Map<String, NativeRow> parseNativeRows(String html) {
        ...
        while(tm.find()){
            String table=tm.group(); String low=clean(table).toLowerCase();
            if(!low.contains("fernkampf")&&!low.contains("reichweite")&&!low.contains("tp/entfernung"))continue;
            List<String> headers=cells(table,"th"); if(headers.isEmpty())continue;
            Matcher rm=Pattern.compile("(?is)<tr\\b[^>]*>(.*?)</tr>").matcher(table);
            while(rm.find()){
                List<String> cs=cells(rm.group(1),"td"); if(cs.size()<2)continue; NativeRow row=new NativeRow();
    ...
    private static List<String> cells(String html,String tag){List<String> out=new ArrayList<String>();Matcher m=Pattern.compile("(?is)<"+tag+"\\b[^>]*>(.*?)</"+tag+">").matcher(html);while(m.find())out.add(clean(m.group(1)));return out;}
    private static void set(NativeRow r,String header,String value){String h=normalize(header).replace(" ","");if(h.contains("fernkampfwaffe")||h.equals("waffe")||h.equals("name"))r.name=value;else if(h.equals("tp"))r.tp=value;else if(h.contains("tp/kk")||h.equals("tpkk"))r.tpkk=value;else if(h.contains("tp/entfernung")||h.contains("tpentfernung"))r.tpEntfernung=value;else if(h.contains("reichweite")||h.equals("rw"))r.reichweite=value;else if(h.equals("lz")||h.contains("ladezeit"))r.lz=value;else if(h.equals("ini")||h.contains("initiative"))r.ini=value;else if(h.equals("wm")||h.contains("waffenmod"))r.mod=value;else if(h.contains("munition"))r.munition=value;else if(h.contains("minbf"))r.minbf=value;else if(h.contains("aktbf"))r.aktbf=value;}
    private static String clean(String s){return s.replaceAll("(?is)<script.*?</script>|<style.*?</style>","").replaceAll("(?is)<[^>]+>"," ").replace("&nbsp;"," ").replace("&amp;","&").replace("&lt;","<").replace("&gt;",">").replaceAll("\\s+"," ").trim();}
